% 2023-05-04 두개의 서로 다른 ROI 영역의 DICE와 IoU를 계산하는 스크립트 개발
% AIM-163 : IoU 계산 요청 (from. 장은지)
% <Input>
...
% output 파일 경로
% exsample output 경로 (실제 분석 시 수정하세요!)
result_path = "E:\git\matlab_source_org\AI_model\10.Dice_IoU\2.result(example)\retulst_Calculate_DICE_ IoU_of_ROI1_and_ROI2";
...
clear dir;
% ROI 1 세트 목록 = DICE&IoU 계산할 샘플목록 (실제 분석 시 수정하세요!)
...
disp(strcat("ROI 파일 ", num2str(numel(list_ROI1_Files)), "개 ROI 1 로딩"))

clear dir;
% ROI 2 세트 목록 = DICE&IoU 계산할 샘플목록 (실제 분석 시 수정하세요!)
...
disp(strcat("ROI 파일 ", num2str(numel(list_ROI2_Files)), "개 ROI 2 로딩"))
...
for file_i = 1:numel(list_ROI1_Files)
    ...
    % BW1이 logical ROI(true or false)가 아니면 Binary로 변환
    if strcmp(string(class(BW1)), "logical")
        BW1_gray = BW1;
        BW1_binary = BW1;
    ...
    % BW2이 logical ROI(true or false)가 아니면 Binary로 변환
    if strcmp(string(class(BW2)), "logical")
        BW2_gray = BW2;
        BW2_binary = BW2;
    else
    ...
    subplot(2,4,[4 8]); imshow(img_imfuse);
    title("ROI 1, 2 - imfuse")

    % subplot을 1개의 jpg 파일로 출력
    print(gcf,'-djpeg','-r300', strcat(result_path,'/', list_ROI1_Files(file_i).name, '_', 'compared', '_DICE_IoU.jpg'));
